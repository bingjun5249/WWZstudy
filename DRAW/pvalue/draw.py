...
p_value = GetPvalue(39.47, 34.01+39.47)
print(p_value)
'''
bkg = 39.47
sig = 34.01
...
plt.fill_between(array, test, where=array>=sig+bkg, color='red', label='P-Value')
plt.plot(test2, color='black', label='Signal + Background', linewidth=2.0)
plt.text(350, 10e-7, '(combined channel)', fontsize=17)
#plt.xlim(0,400)
plt.ylim(10e-13, 10)
plt.legend(fontsize=15, loc='upper right')
plt.yscale('log')
plt.xlabel('Number of Expected Events', fontsize=20, loc='center')
plt.ylabel('Probability', fontsize=20, loc='center')
plt.savefig("total_log")
plt.show()

'''
total_sig = ROOT.Math.gaussian_quantile_c(p_value,1)
print(total_sig)
'''
