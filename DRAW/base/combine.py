...
parser.add_argument('--when', type=str,default='pre',help="--when pre/cut")

args = parser.parse_args()

when = args.when

com_list = ['even','odd','total']
...
def Fourlep_mass(stk_Flep_mass, WWZ_Flep_mass, weight, color, label, com):
	...
	plt.ylabel("Expected Events | 10 GeV", fontsize=20, loc='center')
	plt.legend(fontsize=12, loc='upper right')
	plt.yscale('log')
...
def Jet_PT(stk_jet_pt, WWZ_jet_pt, weight, color, label, com):
	...
	plt.ylabel("Expected Events | 50 GeV", fontsize=20, loc='center')
	plt.legend(fontsize=12, loc='upper right')
	plt.yscale('log')
	plt.savefig("Jet_pt")
	#plt.show()
...
def HT(stk_HT, WWZ_HT, weight, color, label, com):
	...
	plt.ylabel("Expected Events | 40 GeV", fontsize=18, loc='top')
	plt.legend(fontsize=12, loc='upper right')
	plt.savefig("HT")
	#plt.show()
	plt.close()
...
for com in com_list:
	os.chdir('/home/bjpark/WWZ/DRAW/baseline/png/'+when+'/'+com+'/')

	# Data load
	if when == 'pre':
		infile = '/home/bjpark/WWZ/DRAW/baseline/npy/'+when+'_archive/combine_'+com+'.npy'
	else:
		infile = '/home/bjpark/WWZ/DRAW/baseline/npy/'+when+'_archive/history/HT_MET_odd_cut/combine_'+com+'.npy'
