...
#parser.add_argument('--thr', type=float, default='0.98', help='--thr THRESHOLD NUMBER')
...
def Lep_PT(stk_lepPT, WWZ_lepPT, weight, color, label, order):
	...
	plt.hist(stk_lepPT, range=(0,300), bins=30, weights=weight, color=blk, histtype='step', linewidth=0.5, stacked=True)
	plt.hist(stk_lepPT, range=(0,300), bins=30, weights=weight, color=color, label=label, stacked=True)
	plt.hist(WWZ_lepPT, range=(0,300), bins=30, weights=scales['WWZ'], color='red', histtype='step', linewidth=2, label='WWZ')
	plt.title("$\sqrt{s}=14$ TeV, $L=3000$ $fb^{-1}$", fontsize=13, loc='right')
	plt.title("(combined channel)", fontsize=13, loc='left')
	plt.xlim(0,300)
	plt.xticks(fontsize=16)
	plt.yticks(fontsize=16)
	plt.xlabel("" + order + " Lepton $p_{T}$ [GeV]",fontsize=25, loc='right')
...
def Fourlep_PT(stk_FlepPT, WWZ_FlepPT, weight, color, label):

	plt.figure(figsize=(8,8))
	plt.style.use(hep.style.CMS)
	bins = np.linspace(0,50,100)
	plt.hist(stk_FlepPT, range=(0,500), bins=50, weights=weight, color=blk, histtype='step', linewidth=0.5, stacked=True)
	plt.hist(stk_FlepPT, range=(0,500), bins=50, weights=weight, color=color, label=label, stacked=True)
	plt.hist(WWZ_FlepPT, range=(0,500), bins=50, weights=scales['WWZ'], color='red', histtype='step', linewidth=2, label='WWZ')
	plt.title("$\sqrt{s}=14$ TeV, $L=3000$ $fb^{-1}$", fontsize=13, loc='right')
	plt.title("(combined  channel)", fontsize=13, loc='left')
	plt.xlim(0,500)
	plt.xticks(fontsize=16)
	plt.yticks(fontsize=16)
	plt.xlabel("$p_{T,4l}$ [GeV]",fontsize=25, loc='right')
	plt.ylabel("Expected Yield | 10 GeV", fontsize=20, loc='center')
	plt.legend(fontsize=12, loc='upper right')
	plt.yscale('log')
...
def Fourlep_mass(stk_Flep_mass, WWZ_Flep_mass, weight, color, label):
...
def Dilep_mass(stk_dilep_mass, WWZ_dilep_mass, weight, color, label):
	...
	plt.style.use(hep.style.CMS)
	bins = np.linspace(0,50,100)
	plt.hist(stk_dilep_mass, range=(71,111), bins=20, weights=weight, color=blk, histtype='step', linewidth=0.5, stacked=True)
	plt.hist(stk_dilep_mass, range=(71,111), bins=20, weights=weight, color=color, label=label,  stacked=True)
	plt.hist(WWZ_dilep_mass, range=(71,111), bins=20, weights=scales['WWZ'], color='red', histtype='step', linewidth=2, label='WWZ')
	plt.title("$\sqrt{s}=14$ TeV, $L=3000$ $fb^{-1}$", fontsize=13, loc='right')
	plt.title("(combined channel)", fontsize=13, loc='left')
	plt.xlim(71,111)
	plt.xticks(fontsize=16)
...
def MET(stk_MET, WWZ_MET, weight, color, label):
	...
	plt.style.use(hep.style.CMS)
	bins = np.linspace(0,50,100)
	plt.hist(stk_MET, range=(0,250), bins=250, weights=weight, color=blk, histtype='step', linewidth=0.5, stacked=True)
	plt.hist(stk_MET, range=(0,250), bins=250, weights=weight, color=color, label=label, stacked=True)
	plt.hist(WWZ_MET, range=(0,250), bins=250, weights=scales['WWZ'], color='red', histtype='step', linewidth=2, label='WWZ')
	plt.title("$\sqrt{s}=14$ TeV, $L=3000$ $fb^{-1}$", fontsize=13, loc='right')
	plt.title("(combined channel)", fontsize=13, loc='left')
	plt.xlim(0,250)
	plt.xticks(fontsize=16)
	plt.yticks(fontsize=16)
	plt.xlabel("$E_{T}^{miss}$ [GeV]",fontsize=25, loc='right')
	plt.ylabel("Expected Yield | 10 GeV", fontsize=20, loc='center')
...
def W_leptons_mass(stk_wleps_mass, WWZ_wleps_mass, weight, color, label):
	...
	plt.style.use(hep.style.CMS)
	bins = np.linspace(0,50,100)
	plt.hist(stk_wleps_mass, range=(50,150), bins=100, weights=weight, color=blk, histtype='step', linewidth=0.5, stacked=True)
	plt.hist(stk_wleps_mass, range=(50,150), bins=100, weights=weight, color=color, label=label, stacked=True)
	plt.hist(WWZ_wleps_mass, range=(50,150), bins=100, weights=scales["WWZ"], color='red', histtype='step', linewidth=2, label='WWZ')
	plt.title("$\sqrt{s}=14$ TeV, $L=3000$ $fb^{-1}$", fontsize=13, loc='right')
	plt.title("(combined channel)", fontsize=13, loc='left')
	plt.xlim(50,150)
	plt.xticks(fontsize=16)
	...
	plt.ylabel("Expected Yield | 5 GeV", fontsize=20, loc='center')
	plt.legend(fontsize=12, loc='upper right')
	plt.yscale('log')
...
def HT(stk_HT, WWZ_HT, weight, color, label):

	plt.figure(figsize=(8,8))
	plt.style.use(hep.style.CMS)
	bins = np.linspace(0,50,100)
	plt.hist(stk_HT, range=(0,2000), bins=50, weights=weight, color=blk, histtype='step', linewidth=0.5, stacked=True)
	plt.hist(stk_HT, range=(0,2000), bins=50, weights=weight, color=color, label=label, stacked=True)
	plt.hist(WWZ_HT, range=(0,2000), bins=50, weights=scales["WWZ"], color='red', histtype='step', linewidth=2, label='WWZ')
	plt.title("$\sqrt{s}=14$ TeV, $L=3000$ $fb^{-1}$", fontsize=13, loc='right')
	plt.title("(combined channel)", fontsize=13, loc='left')
	plt.xlim(0,2000)
	plt.xticks(fontsize=16)
...
bkg_vari = ['fstlep_pt','fstlep_eta','sndlep_pt','sndlep_eta','trdlep_pt','trdlep_eta','frtlep_pt','frtlep_eta','fourlep_pt','fourlep_mass','dilep_mass','wleps_mass','MET_MET','MT2','jet_pt','HT']#'jet_btag','HT']
...
weight = [scales['ttbar'], scales['ZG'], scales['Z'], scales['ZZZ'], scales['WZZ'], scales['tWZ'], scales['ttbarZ'], scales['ZZ'], scales['WZ']]

# color
#color = ['royalblue','gold','aqua','c','g','orange','coral','darkslategrey','midnightblue']
color = ['midnightblue','darkslategrey','coral','g','c','aqua','royalblue','gold','orange']
blk = ['black','black','black','black','black','black','black','black','black']

# label
#label = ['ZZ', '$t\overline{t}+Z$', 'tWZ', 'WZZ', 'ZZZ', 'WZ', '$Z\gamma$', 'Z', '$t\overline{t}$']
label = ['$t\overline{t}$','$Z\gamma$','Z','ZZZ','WZZ','tWZ','$t\overline{t}+Z$','ZZ','WZ']
...
# four Lepton PT
Fourlep_PT(stk_fourlepPT, WWZ['fourlep_pt'], weight, color, label)

# four Lepton PT
Fourlep_mass(stk_fourlep_mass, WWZ['fourlep_mass'], weight, color, label)
...
# HT
HT(stk_HT, WWZ['HT'], weight, color, label)
