...
branches = ['Electron.PT', 'Electron.Eta', 'Electron.Phi', 'Electron.Charge', 'MuonTight.PT', 'MuonTight.Eta', 'MuonTight.Phi', 'MuonTight.Charge', 'PuppiMissingET.MET', 'PuppiMissingET.Phi', 'JetPUPPI.BTag', 'JetPUPPI.PT', 'JetPUPPI.Phi', 'JetPUPPI.Eta']
...
def Lepton_selection(particles, e_num):
	...
	Electron_lepsel = Electron[channel_mask]
	Muon_lepsel = Muon[channel_mask]
...
def Selection(file_list, e_num):
	...
	for arrays in tqdm(uproot.iterate(flist,branches)):
		...
		wlepmask = (abs(wleptons.mass - 90) > 10)
		...
		# Event counting
		count += len(btag_lep.lep1.PT)
		#print(count)

		# MT2 calculation
		
		mt2 = calMT2(btag_lep, btag_MET)
		mt2 = ak.flatten(mt2)
		...
		try:
			...
			fst_lep = vector.obj(pt=btag_lep.lep1.PT, phi=btag_lep.lep1.Phi, eta=btag_lep.lep1.Eta, mass=0)
			snd_lep = vector.obj(pt=btag_lep.lep2.PT, phi=btag_lep.lep2.Phi, eta=btag_lep.lep2.Eta, mass=0)
			trd_lep = vector.obj(pt=btag_lep.lep3.PT, phi=btag_lep.lep3.Phi, eta=btag_lep.lep3.Eta, mass=0)
			frt_lep = vector.obj(pt=btag_lep.lep4.PT, phi=btag_lep.lep4.Phi, eta=btag_lep.lep4.Eta, mass=0)

			dilep = fst_lep + snd_lep
			wleps = trd_lep + frt_lep
			
			MET_vec = vector.obj(pt=btag_MET.MET, phi=btag_MET.Phi)
			
			Jet_vec = vector.obj(pt=btag_Jet.PT, phi=btag_Jet.Phi, eta=btag_Jet.Eta, mass=0)
			...
			MET_MET = ak.to_numpy(ak.flatten(btag_MET.MET))
			MET_phi = ak.to_numpy(ak.flatten(btag_MET.Phi))
			MT2 = ak.to_numpy(mt2)

			Jet_list = []
			for i in range(len(Jet_vec.pt)): Jet_list.append(Jet_vec.pt[i][0])
			Jet_pt = Jet_list
			Jet_btag = ak.to_numpy(ak.flatten(btag_Jet['BTag']))
