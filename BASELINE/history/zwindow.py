...
def Selection(file_list, e_num):
	...
	for arrays in tqdm(uproot.iterate(flist,branches)):
		...
		zlep1 = vector.obj(pt=lep_z.lep1.PT, phi=lep_z.lep1.Phi, eta=lep_z.lep1.Eta, mass=0)
		zlep2 = vector.obj(pt=lep_z.lep2.PT, phi=lep_z.lep2.Phi, eta=lep_z.lep2.Eta, mass=0)
		Z = zlep1 + zlep2

		zmass_mask = (abs(Z.mass - 90.1876) < 10)
		zwindow_lep = lep_z[zmass_mask]
		ossf_MET = ossf_particles[2]
		zwindow_MET = ossf_MET[zmass_mask]

		print("-----> Z mass window done. : {0}".format(len(zwindow_lep.lep1.PT)))
		if len(zwindow_lep.lep1.PT) == 0: continue

		# Event counting
		count += len(zwindow_lep.lep1.PT)
		#print(count)

		# MT2 calculation
		
		mt2 = calMT2(zwindow_lep,zwindow_MET)
		mt2 = ak.flatten(mt2)
		...
		try:
			...
			fst_lep = vector.obj(pt=zwindow_lep.lep1.PT, phi=zwindow_lep.lep1.Phi, eta=zwindow_lep.lep1.Eta, mass=0)
			snd_lep = vector.obj(pt=zwindow_lep.lep2.PT, phi=zwindow_lep.lep2.Phi, eta=zwindow_lep.lep2.Eta, mass=0)
			trd_lep = vector.obj(pt=zwindow_lep.lep3.PT, phi=zwindow_lep.lep3.Phi, eta=zwindow_lep.lep3.Eta, mass=0)
			frt_lep = vector.obj(pt=zwindow_lep.lep4.PT, phi=zwindow_lep.lep4.Phi, eta=zwindow_lep.lep4.Eta, mass=0)

			dilep = fst_lep + snd_lep
			wleps = trd_lep + frt_lep
			
			MET_vec = vector.obj(pt=zwindow_MET.MET, phi=zwindow_MET.Phi)
			...
			MET_MET = ak.to_numpy(ak.flatten(zwindow_MET.MET))
			MET_phi = ak.to_numpy(ak.flatten(zwindow_MET.Phi))
			MT2 = ak.to_numpy(mt2)
